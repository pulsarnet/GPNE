...
namespace ptn::alloc::vec {
    ...
    template<typename T>
    void RustVec<T>::push_back(const T& value) {
        this->emplace_back(value);
    }

    template<typename T>
    void RustVec<T>::push_back(T&& value) {
        this->emplace_back(std::move(value));
    }

    template<typename T>
    template<typename ...Args>
    void RustVec<T>::emplace_back(Args&&... args) {
        auto size = this->size();
        this->reserve(size + 1);
        ::new (reinterpret_cast<T*>(reinterpret_cast<char*>(this->data()) + size * sizeof(T))) T(std::forward<Args>(args)...);
        this->resize(size + 1);
    }
