...
pub use crate::net::edge::direct::DirectedEdge;
pub use crate::net::edge::inhibitor::InhibitorEdge;
...
impl Edges {
    ...
    pub fn remove_directed(&mut self, from: VertexIndex, to: VertexIndex) -> Option<DirectedEdge> {
        let idx = self
            .directed
            .iter()
            .position(|x| x.begin() == from && x.end() == to)?;
    ...
    pub fn get_directed(&self, from: VertexIndex, to: VertexIndex) -> Option<&DirectedEdge> {
        assert_ne!(
            from.type_, to.type_,
            "'from' and 'to' should have different type"
        );
        self.directed
            .iter()
            .find(|e| e.begin() == from && e.end() == to)
    }

    pub fn get_directed_mut(
        &mut self,
        from: VertexIndex,
        to: VertexIndex,
    ) -> Option<&mut DirectedEdge> {
        assert_ne!(
            from.type_, to.type_,
            "'from' and 'to' should have different type"
        );
    ...
    pub fn add_inhibitor(&mut self, edge: InhibitorEdge) -> bool {
        if self
            .get_inhibitor_mut(edge.place(), edge.transition())
            .is_none()
        {
    ...
    pub fn remove_inhibitor(&mut self, from: VertexIndex, to: VertexIndex) {
        ...
        let idx = self
            .inhibitor
            .iter()
            .position(|x| x.place() == from && x.transition() == to);
        if let Some(idx) = idx {
    ...
    pub fn get_inhibitor(&self, from: VertexIndex, to: VertexIndex) -> Option<&InhibitorEdge> {
        self.inhibitor
            .iter()
            .find(|e| e.place() == from && e.transition() == to)
    }

    pub fn get_inhibitor_mut(
        &mut self,
        from: VertexIndex,
        to: VertexIndex,
    ) -> Option<&mut InhibitorEdge> {
        self.inhibitor
            .iter_mut()
            .find(|e| e.place() == from && e.transition() == to)
    }
    ...
    pub fn remove_relative_to_vertex(&mut self, idx: VertexIndex) {
        ...
        self.inhibitor
            .retain(|e| e.place() != idx && e.transition() != idx);
    }
}
...
mod tests {
    use crate::net::vertex::VertexIndex;
    use crate::net::{DirectedEdge, Edges, InhibitorEdge};

    #[test]
    fn add_edge() {
        let mut edges = Edges::default();

        let directed_t1_p1 = DirectedEdge::new(VertexIndex::transition(1), VertexIndex::place(1));
        let directed_t1_p2 = DirectedEdge::new(VertexIndex::transition(1), VertexIndex::place(2));
    ...
    fn remove_relative_to_vertex() {
        let mut edges = Edges::default();
        let directed_t1_p1 = DirectedEdge::new(VertexIndex::transition(1), VertexIndex::place(1));
        let directed_t2_p2 = DirectedEdge::new(VertexIndex::transition(2), VertexIndex::place(2));
        let inhibitor_p1_t1 = InhibitorEdge::new(VertexIndex::place(1), VertexIndex::transition(1));
        let inhibitor_p2_t2 = InhibitorEdge::new(VertexIndex::place(2), VertexIndex::transition(2));
        ...
        edges.remove_relative_to_vertex(VertexIndex::place(2));
