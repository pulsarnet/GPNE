...
pub fn t_invariant(net: &PetriNet) {
    ...
    let transitions = net
        .transitions()
        .iter()
        .fold(vec![], |mut acc, (key, _transition)| {
            acc.push(format!("t{}", key));
            acc
        });
...
pub fn p_invariant(net: &PetriNet) {
    ...
    let positions = net.places().iter().fold(vec![], |mut acc, (key, _pos)| {
        acc.push(format!("p{}", key));
        acc
    });
...
fn invariant(mut mat_c: DMatrix<i32>) -> DMatrix<i32> {
    ...
    while let Some(pn) = PositiveNegative::new(&mat_c) {
        ...
        if pn.p_cols.is_empty() ^ pn.n_cols.is_empty() {
            // There is a row in which only one set (positive and negative) of elements is not empty
            // Then remove all columns from math and math that are in the p_cols U n_cols union.
            let remove = pn
                .p_cols
                .iter()
